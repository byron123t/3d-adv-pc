...
BASEPATH = './data/kitti_dataset/extracts/'
...
def main():
	...
	np.save('points_full.npy', np.asarray(cloud_full))
	np.save('points_crop.npy', np.asarray(cloud0))

	pd.DataFrame(label_full).to_csv('labels_full.csv', header=None, index=None)
	pd.DataFrame(meta_full).to_csv('metadata_full.csv', header=None, index=None)

	pd.DataFrame(label0).to_csv('labels_crop.csv', header=None, index=None)
	pd.DataFrame(meta0).to_csv('metadata_crop.csv', header=None, index=None)
